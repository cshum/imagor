...
import (
	...
	"github.com/cshum/imagor/imagorurl"
	"go.uber.org/zap"
	"net/http"
	"reflect"
	"strconv"
...
type Processor interface {
	...
	Process(ctx context.Context, buf []byte, p imagorurl.Params, load LoadFunc) ([]byte, *Meta, error)
...
func (app *Imagor) Do(r *http.Request, p imagorurl.Params) (buf []byte, meta *Meta, err error) {
	var cancel func()
	ctx := r.Context()
	...
	if !(app.Unsafe && p.Unsafe) && imagorurl.Sign(p.Path, app.Secret) != p.Hash {
		err = ErrSignatureMismatch
		if app.Debug {
			app.Logger.Debug("sign-mismatch", zap.Any("params", p), zap.String("expected", imagorurl.Sign(p.Path, app.Secret)))
		}
		return
	}
