...
### Basic Upload and Resize

```bash
curl -X POST -F "image=@original.jpg" \
  http://localhost:8000/unsafe/800x600/
```

### Upload with Multiple Transformations

```bash
curl -X POST -F "image=@photo.jpg" \
  http://localhost:8000/unsafe/fit-in/400x300/filters:quality(85):format(webp):sharpen(1)/
```

### Upload with Smart Cropping

```bash
curl -X POST -F "image=@portrait.jpg" \
  http://localhost:8000/unsafe/300x300/smart/filters:fill(white)/
```

## Built-in Upload Form

When accessing upload endpoints in a browser, imagor provides a convenient upload form:
...
The upload form is particularly useful for:
...
fetch('/unsafe/400x300/filters:quality(80)/', {
  method: 'POST',
  body: formData
})
...
with open('image.jpg', 'rb') as f:
    ...
    response = requests.post(
        'http://localhost:8000/unsafe/300x200/filters:format(webp)/',
        files=files
    )
