	...
	for i, loader := range loaders {
		...
		for _, tt := range tests {
			t.Run(fmt.Sprintf("%s-%d", tt.name, i+1), func(t *testing.T) {
				...
				blob1 := NewBlobFromTarget(func(target *Target) error {
					return img1.ExportWebpTarget(target, nil)
				})
				buf1, err := blob1.ReadAll()
				require.NoError(t, err)
				blob2 := NewBlobFromTarget(func(target *Target) error {
					return img2.ExportWebpTarget(target, nil)
				})
				buf2, err := blob2.ReadAll()
				require.NoError(t, err)
