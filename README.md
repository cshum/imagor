...
- `image(imagor_path [, x [, y [, alpha]]])` composites a processed image onto the current image with full imagor transformation support. Unlike `watermark()` which only resizes images, `image()` applies the complete imagor processing pipeline including filters, enabling recursive image composition:
  - `imagor_path` - a complete imagor path with transformations e.g. `/200x200/filters:grayscale()/photo.jpg`
